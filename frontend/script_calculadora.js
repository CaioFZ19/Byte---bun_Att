...
async function carregarCardapio() { 
    ...
    popularSelect("sltPao", itens.filter((item => item.categoria === "pao"))) 
    popularSelect("sltRecheio", itens.filter((item => item.categoria === "recheio"))) 
    popularSelect("sltMolho", itens.filter((item => item.categoria === "molho")))
} 
...
btnCalcularPedido.addEventListener("click", async() => { 
    const pedido = { 
        pao: document.querySelector("#sltPao").value,
        recheio: document.querySelector("#sltRecheio").value, 
        molho: document.querySelector("#sltMolho").value 
    } 

    try { 
        const resposta = await fetch (
            `${URL_API}/pedido`, { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify(pedido) }
        )
